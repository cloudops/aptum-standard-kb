---
title: "Generate a Aptum Portal API key"
slug: aptum-portal-api-key
---


When working with the Aptum Portal API, you will need to generate an API key for use with your code.  API keys provide a convenient method for your application to identify itself to a service when making calls to the service's API.

Any Aptum Portal user may generate an API key.  A user's API keys will have the same level of privilege that the user has.  There is no limit to the number of API keys that a user may generate.  It is recommended to take advantage of this by generating an API key for each application that will be accessing the system.
...
![API credentials screen](img/aptum-portal-api-key-en-01.png)
...
![API key generated](img/aptum-portal-api-key-en-02.png)
