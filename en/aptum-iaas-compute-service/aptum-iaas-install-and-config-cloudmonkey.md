---
title: "Aptum IaaS: Install and configure CloudMonkey"
slug:  aptum-iaas-install-and-config-cloudmonkey
---


Apache CloudMonkey is a Command Line Interface written in Python to interact with Aptum IaaS APIs.

This tool is useful to automate virtual infrastructure in Aptum IaaS or to manage it thru the CLI. Apache CloudMonkey can be installed on various operating systems such as Linux and macOS, it only requires Python 2.7+ to be installed.
...
### Connect to the Aptum IaaS service

Launch the CLI with the command `cloudmonkey`, then use the CLI command `sync` to confirm CloudMonkey is connected to the Aptum IaaS API.
